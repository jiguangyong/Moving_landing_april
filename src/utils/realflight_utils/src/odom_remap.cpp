...
private:
    ...
    apriltag_ros::AprilTagDetectionArray empty;
    ...
    void visionCallback(const apriltag_ros::AprilTagDetectionArray::ConstPtr &transform){ 
        if(transform->detections != empty.detections){
    ...
    Eigen::Vector3d fromQuaternion2Eular(Eigen::Quaterniond q)
    {
        Eigen::Vector3d car_Euler;
        double yaw = atan2(2 * (q.x()*q.y() + q.w()*q.z()), q.w()*q.w() + q.x()*q.x() - q.y()*q.y() - q.z()*q.z());
        double roll = atan2(2 * (q.y() * q.z() + q.w() * q.x()),q.w() * q.w() - q.x() * q.x() - q.y() * q.y() + q.z() * q.z());
    ...
    void gtruth_const_bias_cal()
    {
        ...
        else
        {
            ...
            // // Eigen::AngleAxisd pitch(Eigen::AngleAxisd(car_rpy_bias.y,Eigen::Vector3d::UnitY()));
            // // Eigen::AngleAxisd yaw(Eigen::AngleAxisd(car_rpy_bias.z,Eigen::Vector3d::UnitZ()));
            // Eigen::Quaterniond cur_qua_bias;
            // cur_qua_bias.w()= car_odom_sim.pose.pose.orientation.w;
            // cur_qua_bias.x()= car_odom_sim.pose.pose.orientation.x;
            // cur_qua_bias.y()= car_odom_sim.pose.pose.orientation.y;
            // cur_qua_bias.z()= car_odom_sim.pose.pose.orientation.z;
    ...
    //     }
    // }
    void car_motion_cal_april(Eigen::Quaterniond& qua, Eigen::Vector3d& pos, Eigen::Vector3d& lin_vel, Eigen::Vector3d& ekf_error)
    {
    	int sour_last = source;
        ...
        Eigen::Vector3d l_v(car_odom_sim.twist.twist.linear.x, car_odom_sim.twist.twist.linear.y, car_odom_sim.twist.twist.linear.z);
        lin_vel = car_qua_bias.inverse() * l_v; 
        lin_vel[2] = 0.0;
        // lin_vel = gtruth_qua_bias.inverse() * l_v; 
        // lin_vel = l_v;
        if(use_vis != 1 || (vision_position.x() == 0 && vision_position.y() == 0 && vision_position.z() == 0 && vision_ori.w() == 0 && vision_ori.x() ==0 && vision_ori.y() == 0 && vision_ori.z()==0))
        {
            ...
            // Eigen::Vector3d p_bias(CU_pos_init_differ.x, CU_pos_init_differ.y, CU_pos_init_differ.z);
            Eigen::Vector3d p_bias(0,0,0);
            // Eigen::Vector3d l_v(0,0,0);
            // Eigen::Vector3d a_v(0,0,0);
            // Eigen::Vector3d car_ypr;

            // car_ypr = fromQuaternion2Eular(car_q1);
            // Eigen::AngleAxisd roll(Eigen::AngleAxisd(car_odom_sim.roll,Eigen::Vector3d::UnitX()));
            // Eigen::AngleAxisd pitch(Eigen::AngleAxisd(car_odom_sim.pitch,Eigen::Vector3d::UnitY()));
            // Eigen::AngleAxisd yaw(Eigen::AngleAxisd(car_odom_sim.yaw,Eigen::Vector3d::UnitZ()));

            car_orientation.w() =  car_odom_sim.pose.pose.orientation.w;
            car_orientation.x() =  car_odom_sim.pose.pose.orientation.x;
            car_orientation.y() =  car_odom_sim.pose.pose.orientation.y;
            car_orientation.z() =  car_odom_sim.pose.pose.orientation.z;
            qua = car_orientation;
    ...
    void gps_odom_pub(const ros::TimerEvent& time_event)
    {
        if(gtruthsubTri && carodomsubTri)
        {
            ...
            else if(seq == 300)
            {
                ...
                car_qua_bias.w() = car_qua_bias.w() / bias_c;
                car_qua_bias.x() = car_qua_bias.x() / bias_c;
                car_qua_bias.y() = car_qua_bias.y() / bias_c;
                car_qua_bias.z() = car_qua_bias.z() / bias_c;
                ...
                std::cout<<"gtruth_qua_bias.w() = "<<gtruth_qua_bias.w()<<" gtruth_qua_bias.x() = "<<gtruth_qua_bias.x()<<" gtruth_qua_bias.y() = "<<gtruth_qua_bias.y()<<" gtruth_qua_bias.z() = "<<gtruth_qua_bias.z()<<std::endl;
                std::cout<<"car_qua_bias.w() = "<<car_qua_bias.w()<<" car_qua_bias.x() = "<<car_qua_bias.x()<<" car_qua_bias.y() = "<<car_qua_bias.y()<<" car_qua_bias.z() = "<<car_qua_bias.z()<<std::endl;
            }
            else
            {
                /* uav publish */
                if( (abs(gtruth.header.stamp.toSec() - gtruth_time_l) < 0.04))
                {
                    ...
                    odomPub.publish(odomMsg);
                        ...
                        if(calibration)
                        {
                            ...
                            if(issimulation)
                             {
                                car_motion_cal_april(car_qua, car_pos, car_vel, ekf_error);
        ...
        switch (odom_source) {
            ...
            case 1:
                ...
                else
                {
                    ...
                    visionsub = nh.subscribe("/tag_detections", 1, &odomRemap::visionCallback, this);//获取apriltag的位置和姿态
                    
                    
                    vision_getPub = nh.advertise<std_msgs::Float64>("/vision_received", 1);

                    ekf_param_init();
                    car_odomSub = nh.subscribe("/ugv_0/odom", 1, &odomRemap::sim_car_odom_Callback, this,
                                        ros::TransportHints().tcpNoDelay());
                }
